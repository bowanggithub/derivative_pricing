...
SimpleBinomialTree::SimpleBinomialTree(double Spot_, const Parameters& r_,
        const Parameters& d_, double Volatility_, unsigned long Steps_,
        double Time_)
    : Spot(Spot_), r(r_), d(d_), Volatility(Volatility_),Steps(Steps_),
...
void SimpleBinomialTree::BuildTree()
{
    ...
    for (unsigned long i=0; i<= Steps;i++)
    {
        ...
        double movedLogSpot = InitialLogSpot + r.Integral(0.0,thisTime)
    ...
    for (unsigned long i=1; i<=Steps; i++)
    {
                ...
                j<=static_cast<long>(index); j=j+2,k++)
        {
            ...
            TheTree[index][k].second=
                TheProduct.PreFinalValue(Spot,ThisTime, futureDicountedValue);
        }
    }
